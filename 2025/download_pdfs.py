#!/usr/bin/env python3
"""
Script to download all PDFs from markdown files and create a comprehensive index
"""
...
class PDFDownloader:
    ...
    def load_existing_index(self):
        """Load existing index.json to avoid re-downloading files"""
        ...
        if index_path.exists():
            try:
                ...
                if 'downloaded_files' in data:
                    self.downloaded_files = data['downloaded_files']
                    print(f"📋 Loaded {len(self.downloaded_files)} existing downloads from index")
                
                # Load existing failed downloads to avoid retrying permanently failed files
                if 'failed_downloads' in data:
                    self.failed_downloads = data['failed_downloads']
                    print(f"⚠️  Found {len(self.failed_downloads)} previously failed downloads")
                ...
                with open(file_path, 'rb') as f:
                    ...
                    if first_bytes == b'%PDF' and file_path.stat().st_size > 1000:
                        ...
                        with self.lock:
                            ...
                            # Update existing entry with current info if needed
                        ...
                        print(f"🔄 Invalid existing file, re-downloading: {file_path.name}")
    ...
    def process_markdown_files(self):
        """Process all markdown files and create download tasks"""
        ...
        for md_path in md_files:
            ...
            folder_parts = list(relative_path.parts[:-1])  # Exclude index.md
        ...
        return all_downloads, folder_structure

    def download_all_files(self, downloads):
        ...
        for download in downloads:
            ...
            if str(file_path) in self.downloaded_files:
                if file_path.exists():
                    print(f"⏭️  Already in index: {file_path.name}")
                    continue
                else:
                    # File in index but doesn't exist, need to re-download
                    print(f"🔄 File in index but missing, re-downloading: {file_path.name}")
                    files_to_download.append(download)
            else:
                files_to_download.append(download)
        
        total_files = len(downloads)
        skipped_count = total_files - len(files_to_download)
        
        print(f"🚀 Total files found: {total_files}")
        print(f"⏭️  Files already downloaded: {skipped_count}")
    ...
    def create_index_json(self, folder_structure):
        """Create comprehensive index.json file"""
        # Filter out failed downloads that are now successful
        current_failed = [f for f in self.failed_downloads if str(Path(f['file_path'])) not in self.downloaded_files]
        
        index_data = {
            "generated_at": time.strftime("%Y-%m-%d %H:%M:%S"),
            "total_downloaded": len(self.downloaded_files),
            "total_failed": len(current_failed),
            "newly_downloaded_this_run": self.newly_downloaded,
            "skipped_existing_this_run": self.skipped_files,
            "folder_structure": folder_structure,
            "downloaded_files": self.downloaded_files,
            "failed_downloads": current_failed,
            "statistics": {
                "successful_downloads": len(self.downloaded_files),
                "failed_downloads": len(current_failed),
    ...
    def run(self):
        ...
        downloads, folder_structure = self.process_markdown_files()
        ...
        # Download all files
        self.download_all_files(downloads)
        
        # Create index
        index_data = self.create_index_json(folder_structure)
        
        # Summary
        total_files = len(self.downloaded_files)
        current_failed = len([f for f in self.failed_downloads if not any(
            str(Path(f['file_path'])) in self.downloaded_files for f in [f]
        )])
        
        print(f"\n📊 Download Summary:")
        print(f"📁 Total files processed: {len(downloads)}")
        print(f"✅ Total successful: {total_files}")
        print(f"🔄 Newly downloaded: {self.newly_downloaded}")
        print(f"⏭️  Skipped (existing): {self.skipped_files}")
        print(f"❌ Currently failed: {current_failed}")
        print(f"📁 Total size: {index_data['statistics']['total_size_bytes'] / (1024*1024):.2f} MB")
        
        if current_failed > 0:
            print(f"\n❌ Current Failed Downloads:")
            failed_to_show = [f for f in self.failed_downloads if not any(
                str(Path(f['file_path'])) in self.downloaded_files for f in [f]
            )]
            for failed in failed_to_show[:10]:  # Show first 10
                print(f"  - {failed['book_name']} (Link {failed['link_number']}): {failed['error'][:100]}")
            if len(failed_to_show) > 10:
                print(f"  ... and {len(failed_to_show) - 10} more")
        
        success_rate = (total_files / len(downloads)) * 100 if downloads else 0
...
def main():
    ...
    downloader = PDFDownloader(
        ...
        retry_count=2   # Reduced from 3 to 2 as requested
    )
    downloader.run()
